...
HandleNickname.prototype.bindEvents = function () {
  this.socket.on("changeNickname", this.handleChange.bind(this));
...
HandleNickname.prototype.sendNickname = function () {
  if (this.enteredRoom) {
    this.socket.emit("nicknameSuccess", { nickname: this.nickname });

    this.allSockets.emit("newNickname", {
      id: this.socket.id,
    ...
    this.socket.emit("enterRoom", {
      allNicknames: this.nicknameManager.nicknameMap,
      nickname: this.nickname
    });
...
HandleNickname.prototype.leaveRoom = function () {
  ...
  this.socket.broadcast.emit("guestLeft", {
    id: this.socket.id,
    nickname: this.nickname
  });

  this.nicknameManager.delete(this.socket);
};
